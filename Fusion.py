...
part_terrain = st.sidebar.slider("Part du terrain (%)", 0, 100, 15)
apport = st.sidebar.number_input("Apport (€)", value=20000)
frais_dossier = st.sidebar.number_input("Frais de dossier (€)", value=1000)
frais_agence = st.sidebar.number_input("Frais d'agence (€)", value=5000)
montant_travaux = st.sidebar.number_input("Travaux (€)", value=15000)
frais_garantie = st.sidebar.number_input("Frais de garantie (€)", value=1000)
frais_tiers = st.sidebar.number_input("Frais de tiers (€)", value=500)
mobilier = st.sidebar.number_input("Mobilier (€)", value=3000)

duree_annees = st.sidebar.slider("Durée prêt (années)", 5, 30, 20)
taux_interet = st.sidebar.number_input("Taux d'intérêt (%)", value=2.0)
taux_assurance = st.sidebar.number_input("Taux assurance emprunteur (%)", value=0.3)
differe_mois = st.sidebar.slider("Différé (mois)", 0, 24, 0)

charges_copro = st.sidebar.number_input("Charges copropriété (€)", value=1000)
assurance_habitation = st.sidebar.number_input("Assurance habitation (€)", value=300)
assurance_gli = st.sidebar.number_input("Assurance GLI (€)", value=250)
taxe_fonciere = st.sidebar.number_input("Taxe foncière (€)", value=900)
frais_entretien = st.sidebar.number_input("Entretien (€)", value=400)
frais_compta = st.sidebar.number_input("Comptabilité (€)", value=600)
frais_bancaires = st.sidebar.number_input("Frais bancaires (€)", value=100)
gestion_locative = st.sidebar.number_input("Gestion locative (€)", value=0)
taxe_habitation = st.sidebar.number_input("Taxe d'habitation (€)", value=0)

loyer_mensuel_hc = st.sidebar.number_input("Loyer mensuel HC (€)", value=850)
vacance_locative_mois = st.sidebar.slider("Vacance locative (mois)", 0, 12, 1)
tmi = st.sidebar.slider("TMI (%)", 0, 45, 30)
...
if st.sidebar.button("Passer à la SCI à l'IS"):
...
mobilier = st.number_input("Montant mobilier (€)", value=3000)
...
assurance = st.number_input("Assurance PNO (€)", value=300)
assurance_gli = st.number_input("Assurance GLI (€)", value=250)
taxe_fonciere = st.number_input("Taxe foncière (€)", value=900)
frais_entretien = st.number_input("Frais d'entretien (€)", value=400)
frais_compta = st.number_input("Frais comptabilité (€)", value=600)
